...
for combo in combinations:
    ...
    outfile = '../mainstream-analysis/output/streamer/scheduler/distributed/%s/%s/%s-%s-500' % (metric, basenet, metric, file_suffix)
    logfile = 'log/distributed/%s-%s-%s-500-mainstream-simulator.out' % (basenet,metric,file_suffix)
    
    run_str = 'sem -j+0 python src/scheduler/run_scheduler_simulator.py %i %s -m %s --datasets %s > %s' % (max_num_apps, outfile, metric, datasets, logfile)
    print run_str
